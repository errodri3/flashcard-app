...
function App() {
  ...
  const handleGuess = () => {
    ...
    const correct = normalize(cards[cardOrder[currentIndex]].back);
  ...
  const handleMaster = () => {
    const masteredId = cards[cardOrder[currentIndex]].id;
    setMasteredIds((prev) => [...prev, masteredId]);
    setCardOrder((prev) => prev.filter((i) => cards[i].id !== masteredId));
    setCurrentIndex(0);
    setFeedback("");
    setUserGuess("");
  };
  ...
  return (
    <div className="app">
      ...
      <div className="header">
        ...
        <p className="count">Cards remaining: {cards.length - masteredIds.length} | Mastered: {masteredIds.length}</p>
      </div>
      ...
      <Card
        key={cards[cardOrder[currentIndex]].id}
        card={cards[cardOrder[currentIndex]]}
        color={categoryColors[cards[cardOrder[currentIndex]].category]}
      />

      <button className="shuffle-btn" onClick={handleShuffle}>🔀 Shuffle</button>
      <button className="master-btn" onClick={handleMaster}>⭐ Mark as Mastered</button>
